...
public class PlayerController : MonoBehaviour
{
    ...
    void Update()
    {
        if (_powerUpTimer > 0)
        {
            ...
            if (_powerUpTimer <= 0)
            {
                Weapon = _originalWeapon; // Süre bitti, eski silaha dön
            }
        }
        HandleMovement();
        HandleFire();
    ...
    public void ApplyWeaponPowerUp(Weapon newWeapon, float duration)
    {
        if (_powerUpTimer <= 0) // Eğer zaten bir power-up aktif değilse orijinali sakla
        {
            _originalWeapon = Weapon;
        }
        
        Weapon = newWeapon;
        _powerUpTimer = duration; // Süreyi başlat (yeniden alınca süre tazelenir)
    }
